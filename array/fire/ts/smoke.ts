...
class Smoke{
    ...
    public color: string;
    constructor(canvas:HTMLCanvasElement,context:CanvasRenderingContext2D,x:number,y:number,no:number)//,vxrange:number,vymin :number,vymax :number,radiusmin:number,radiusmax:number,color:string)
     { 
      this.context=context;
      this.x=x;
      this.y=y;
      this.no= this.noofparticals;
      this.canvas=canvas;
      this.vxrange=1;
      this.vymin=1;
      this.vymax=5;
